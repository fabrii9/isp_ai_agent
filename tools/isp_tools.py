...
class IspTools:
    ...
    def _only_digits(self, s: str) -> str:
        """Retorna solo los dígitos de un string."""
        return ''.join(c for c in str(s) if c.isdigit())

    def _resolve_partner(self, partner, verified_partner_id=None):
        """
        Resuelve el partner real a usar en una tool.

        Si viene verified_partner_id (retornado por verify_identity), lo usa.
        De lo contrario usa el partner del canal WhatsApp.
        """
        if verified_partner_id:
            try:
                real = self.env['res.partner'].sudo().browse(int(verified_partner_id))
                if real.exists():
                    return real
            except Exception:
                pass
    ...
    def execute_verify_identity(self, partner, vat=None, **kwargs) -> dict:
        """
        Busca en TODOS los partners el que tenga el DNI/CUIT ingresado.
        El número de WhatsApp es irrelevante — la búsqueda es global.

        :param vat: DNI o CUIT ingresado por el cliente (puede tener guiones/espacios)
        :return: {"verified": bool, "verified_partner_id": int, "partner_name": str}
        """
        if not vat:
            return {'verified': False, 'message': 'No se proporcionó DNI/CUIT.'}

        input_digits = self._only_digits(vat)
        if not input_digits:
            return {'verified': False, 'message': 'El valor ingresado no es válido.'}

        # Buscar todos los partners que tengan un VAT registrado
        # Odoo puede almacenar el VAT con o sin prefijo de país (AR, etc.)
        ...
        found = None
        for p in partners:
            if self._only_digits(p.vat or '') == input_digits:
                found = p
                break
        ...
        if not found:
            return {
                'verified': False,
                'message': 'No se encontró ningún cliente con ese DNI/CUIT en el sistema.',
            }

        return {
            'verified': True,
            'verified_partner_id': found.id,
            'partner_name': found.name,
            'message': f'Identidad verificada. Cliente: {found.name}.',
        }
    ...
    def execute_check_debt(self, partner, verified_partner_id=None, **kwargs) -> dict:
        """
        ...
        :param verified_partner_id: ID retornado por verify_identity (tiene prioridad sobre partner)
        :return: {"balance": float, "has_debt": bool, "has_credit": bool, "currency": str}
        """
        partner = self._resolve_partner(partner, verified_partner_id)
        if not partner:
    ...
    def execute_list_overdue_invoices(self, partner, verified_partner_id=None, limit=5, **kwargs) -> dict:
        """
    ...
    def execute_generate_payment_link(self, partner, verified_partner_id=None, invoice_id=None, **kwargs) -> dict:
        """
    ...
    def execute_create_ticket(self, partner, verified_partner_id=None, subject=None, description=None,
                              ticket_type=None, **kwargs) -> dict:
        """
        Crear un ticket de soporte/reclamo en el módulo Helpdesk de Odoo.

        :param verified_partner_id: ID retornado por verify_identity (opcional pero recomendado)
